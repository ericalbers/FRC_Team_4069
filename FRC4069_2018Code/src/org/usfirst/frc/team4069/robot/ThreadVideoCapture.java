...
public class ThreadVideoCapture implements Runnable {
	...
	public Mat[] FrameQueue1;

	public int FrameInIndex1 = 0;
	public int FrameOutIndex1 = 0;
	...
	private VideoCapture vcap, vcap1;
	...
	private void InitCapture() {
		...
		//vcap1 = new VideoCapture();
		//if (vcap1 == null) {
//			System.out.println("vcap 1 is null");
	//	}
		if (vcap == null) {
		...
		case VCT_USB:
			 SetupUSBCamera();
//			SetupAUSBCamera(vcap, 0);
		//	SetupAUSBCamera(vcap1, 1);
			break;
		case VCT_IP:
	...
	/*
	 * Read a frame into framequeue, update indexes allows upto 4 frames to be
	 * stored. IF no frames have been removed, update last frame location with new
	 * frame
	 * 
	 */
	private void Capture() {
		while ((true) && (mKeepRunning == 1)) {
			if (mEnabled) {
				...
				boolean state = vcap.read(FrameQueue[FrameInIndex]); // FrameInIndex is always 'safe' to write too by
																		// definition
				if (state == false) {
	...
	public Mat GetFrame() 
	{
		...
		return null; // no frames available if queue has been drained,
						// we could keep last frame sent and resend it, but calling routine could do
						// that
	}// GetFrame

	private void SetupAUSBCamera(VideoCapture v, int address) {
		int videoStreamAddress = 0;
		System.out.println("Trying to connect to Camera stream... at: " + address);
		int count = 1;

		while ((!v.open(address)) && (mKeepRunning == 1)) {

			System.out.println("Error connecting to camera stream at address " + address + ", retrying " + count);
			count++;
			try {
				Thread.sleep(1000);
			} catch (InterruptedException e) {
		...
		v.set(Videoio.CAP_PROP_FRAME_WIDTH, 320);
		v.set(Videoio.CAP_PROP_FRAME_HEIGHT, 200);
		...
		if (!v.set(Videoio.CAP_PROP_EXPOSURE, 0)) {
			System.out.println("Error prop exposure");
		}

		if (!v.set(Videoio.CAP_PROP_BRIGHTNESS, 0)) // .1)) //.1)) //; //CV_CAP_PROP_BRIGHTNESS, 1);
		{
			System.out.println("Error brightness");
		}
		if (!v.set(Videoio.CAP_PROP_CONTRAST, -1)) // ; //CV_CAP_PROP_CONTRAST, 0);
		{
			System.out.println("Error contrast");
		}
		System.out.println("vcap width = " + v.get(Videoio.CAP_PROP_FRAME_WIDTH));

		System.out.println("vcap height=" + v.get(Videoio.CAP_PROP_FRAME_HEIGHT));
